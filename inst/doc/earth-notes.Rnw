...
Earth's pruning pass uses the \texttt{leaps}~\cite{leaps} package
...
There are other
reasons why the actual number of terms may be less than \texttt{nk}:
...
If \texttt{y} has \texttt{k} columns
...
a tag \texttt{lvol=} to name a column.
\vspace{-4mm}
\begin{verbatim}
...
see Section 4.1 of the FDA paper (Hastie, Tibshirani, and Buja~\cite{Hastie94}).
...
Because of the masking problem, we mention you might consider FDA for
...
on the right hand size of a model with an intercept would have no first ``A'' column
(to prevent linear dependencies on the right side of the model formula).
...
% TODO \texttt{earth} should have an argument to force variables into the model.
...
The black and the red lines are very close.
...
\begin{SCfigure}[][b]
  \begin{centering}
  % \includegraphics[width=.55\textwidth, clip, trim=0mm 90mm 87mm 0mm, keepaspectratio]{figs/cv-detailed.pdf} % trim l b r t
...
\newline
\newline
 \textit{The pink lines are the} \texttt{oof.rsq}'s \textit{for each fold.
The thick red line is the mean of the pink lines.
\newline
\newline
% The black dots are the out-of-fold R-Squareds for the
% models at each fold, selected as usual by the GCV of the training (in-fold) data
% i.e.,
The black dots show the} \texttt{cv.rsq}'s \textit{for each fold.
The} \texttt{cv.rsq} \textit{printed by} \texttt{summary.earth}
\textit{is the mean of the vertical position of these dots.
\newline
\newline
\newline
\newline
...
The names used for these, \texttt{cv.rsq} and \texttt{oof.rsq}, are
somewhat arbitrary, but used consistently in \texttt{earth}'s code and
documentation.
\vspace{-3mm}
\begin{enumerate}[(i)]

\item The \texttt{cv.rsq} is primarily for model assessment,
i.e., estimating generalization ability.
The \texttt{cv.rsq} is printed by \texttt{summary.earth},
but by default not plotted by \texttt{plot.earth}.

A \texttt{cv.rsq} is first calculated for each fold.
...
The model is selected as usual using the GCV of the training (in-fold)
data.  The \texttt{cv.rsq} printed by \texttt{summary.earth}
is the mean of these per-fold \texttt{cv.rsq}'s.

\item The \texttt{oof.rsq}'s are primarily for model selection,
i.e., for selecting the best number of terms.
They aren't printed by \texttt{summary.earth}, but by default are plotted
by \texttt{plot.earth} (Figure~\ref{figs/cv} on page~\pageref{figs/cv}).

The \texttt{oof.rsq} is calculated for every model size in each fold.
...
Note 1: Earth does not actually use cross-validation for automatic
model selection (unlike, say, \texttt{rpart}).
Earth's backward pass with GCVs is always used for that (in the
current implementation at least; there is not yet a \texttt{pmethod="cv"} option).
You can however use \texttt{plot.earth} to see the number of
terms that would be estimated by cross-validation.

Note 2: The \texttt{oof.rsq}'s are calculated only when \texttt{keepxy=TRUE},
...
Each of these is measured on the test (out-of-fold) set for each fold, and
...
not the unknown deviation across samples.
...
(and many other) references: Pearce and Ferrier~\cite{Pearce},
...
\begin{SCfigure}[][b]
  \begin{centering}
  \includegraphics[width=.6\textwidth, clip, trim=0mm 90mm 87mm 0mm, keepaspectratio]{figs/cv.pdf} % trim l b r t
  \caption{\textit{Five-fold cross-validation of an \texttt{earth} model.
  }
\newline
\newline
...
(Figure~\ref{figs/cv}, further discussion in Section~\ref{sec-cv-and-plot}):
\vspace{-4mm}
\begin{verbatim}
    fit <- earth(survived ~ ., data=etitanic, degree=2,
...
\begin{SCfigure}
  ...
  \caption{\textit{Comparing the models at three cross-validation folds.
  }
\newline
\newline
\newline
...
For the curious, \texttt{plot.earth.models} can be used
to compare the models built at each fold. Example (Figure~\ref{figs/cv-folds}):
\vspace{-4mm}
\begin{verbatim}
    fit <- earth(survived ~ ., data=etitanic, degree=2,
                 nfold=3, keepxy=TRUE)
...
Figure~\ref{figs/esl-fig71} is reproduced from Figure~7.1 in Hastie
et al.~\cite{ESL}.
The figure was created from models built with 100 training sets
generated synthetically.

Figure~\ref{figs/cv-example} is an example along the same lines.
...
Most of the pale lines are truncated because the maximum number of
terms generated by \texttt{earth} for the fold models is less than
the 9 terms in the full model.
Many of the arguments in the call to \texttt{plot.earth} above simply remove
...
Much of the variation of the pink curves is due to the
...
model (set \texttt{col.vline} and \texttt{col.oof.vline} to see this, not shown).
A larger number of terms would have been selected with \texttt{degree}
equal to 2, which is actually more appropriate for this data.
...
cross-validating \texttt{earth} models, and your feedback would be appreciated.
The chapter is a general discussion, not limited to \texttt{earth} models.
...
Things change with more flexible models (for example, if we are doing
...
(You can ignore the code for now and just look at the figure.)
...
subset of the variables to include in the model,
then cross-validate the \texttt{earth} model (with \texttt{nfold}) using only that
subset.\footnote[1]{The ``parameter'' being tuned here is
...
If the cross-validation R-Squared is used to select a
model,\footnote[2]{However the \texttt{earth} package does not provide
facilities to do that (Section~\ref{sec-two-kinds} Part 2).} then the
test R-Squared quoted for the selected model must be
\emph{recalculated} for that model using independent data.
The cross-validation R-Squared used to select a model cannot be
quoted as the R-Squared of that model --- that would be conflating the
...
The simplest example is an intercept-only model.  This will give a
negative \texttt{rsq} on test data, unless the training data and test
data happen to have the same mean.
When we calculate \texttt{rsq} on the test data, the intercept-only model
predicts the mean of the \emph{training} data.
...
and \newline
\texttt{rsq = 1 - rss / tss} will be negative.
...
However, after pruning the model will always end up with a
non-negative \texttt{GRSq}.
...
Watch the \texttt{GRSq} take a nose dive in this example
(Figure~\ref{figs/negative-grsq}):
\vspace{-4mm}
\begin{verbatim}
...
From  the formula for the GCV
\vspace{-4mm}
\begin{verbatim}
    GCV = RSS / (nobs * (1 - nparams / nobs)^2)),
\end{verbatim}
\vspace{-3mm}
we see that the GCV increases and then decreases if \texttt{nparams /
nobs} approaches and then exceeds~1 as terms are added to the model.
To prevent this undesirable behavior, if \texttt{nparams / nobs >= 1} then
...
finally adds add 1 for the intercept.
...
\subsection{Why do I get fewer terms than \texttt{nk}, even\newline
...
\subsection{Why do I get fewer terms than \texttt{nprune}?}
\label{faq11}
...
You can work around this because you will get exactly \texttt{nprune} if
you specify \texttt{penalty=-1}.
...
that perfectly separates the classes.
