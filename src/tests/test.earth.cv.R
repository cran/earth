...
set.seed(427)
a.err <- earth(survived~., data=etitanic, degree=1, nfold=3, keepxy=FALSE)
try(plot(a.err$cv.list[[1]])) # expect Error : cannot plot a$cv.list[[1]] because its $residuals field is NULL

# test plot.earth with cross-validated models (example from help page)
set.seed(427)
a.help <- earth(survived~., data=etitanic, trace=1, degree=2, nfold=5, keepxy=TRUE)
print.stripped.earth.model(a.help, "a.help")
plot(a.help) # the full model
plot(a.help$cv.list[[3]])

# test various options
old.par <- par(mfrow=c(2,2), mar=c(4, 3.2, 3, 3), mgp=c(1.6, 0.6, 0), par(cex = 0.8))

plot(a.help, which=1, main="plot.model.selection\ncol.oof.rsq=c(\"red\", \"green\")",
     col.oof.rsq=c("red", "green"), do.par=F)

plot(a.help, which=1, col.oof.rsq=0, col.npreds="gray", lty.npreds=1,
     main="col.oof.rsq=0 col.npreds=gray", do.par=F)

plot(a.help, which=1, main="col.infold.rsq=lightblue",
...
plot.earth.models(a$cv.list, main="plot.earth.models with cross validated models")
